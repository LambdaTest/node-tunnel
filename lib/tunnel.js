...
const TunnelBinary = require('./tunnel_binary')
...
const os = require('os')
let logger
let httpTunnelConfig
const Config_ = require('./config')
const package_ = require('../package.json')
const packageVersion = package_.version
const packageName = package_.name
const usingPorts = []
let globalForceRetries = 0
          ...
          verifyToken_(options, function (e, res) {
            ...
            getBinaryPath_(self, options, function (binaryPath) {
              ...
              if (options.detachedMode === 'true') {
                ...
                const tunnelStatus = runBinaryV2_(self, maxBinaryExecutionRetries)
                if (tunnelStatus) {
  ...
  if (retries >= 0) {
    ...
    addArguments_(self, function (e, binaryArguments) {
      ...
      self.proc.stderr.pipe(split()).on('data', function (data) {
        ...
        if (!isCallback) {
          const logFileIndex = binaryArguments.indexOf('--logFile')
          if (logFileIndex === -1) {
            console.log(
...
function verifyToken_ (options, fnCallback) {
  try {
    ...
    const _httpAuthUrl = urlParse.parse(httpTunnelConfig.jsonResponse.AuthUrl)
    const reqOptions = {
...
function killRunningProcess_ (that, fnCallback) {
  ...
  try {
    ...
    if (that.proc && that.infoAPIPort) {
      const url = 'http://127.0.0.1:' + that.infoAPIPort + '/api/v1.0/stop'
      const parseURL = urlParse.parse(url)
      const reqOptions = {
  ...
  try {
    ...
    getFreePort_(options, 5, function (e, port) {
      if (typeof port === 'number') {
        ...
        const tunnelNameIndex = binaryArgs.indexOf('--tunnelName')
        const hostnamePort = os.hostname() + port
        if (tunnelNameIndex === -1) {
          binaryArgs.push('--tunnelName')
          binaryArgs.push(hostnamePort)
        }
        // add logfile name if not given.
        const logFileIndex = binaryArgs.indexOf('--logFile')
        if (logFileIndex === -1) {
          binaryArgs.push('--logFile')
...
function getBinaryPath_ (that, options, fnCallback) {
  ...
  if (typeof that.binaryPath === 'undefined') {
    ...
    const conf = { user: options.user, key: options.key }
...
function retryTunnelName_ (self, infoAPIPort, retries, fnCallback) {
  try {
    ...
    if (retries >= 0) {
      ...
      const url = 'http://127.0.0.1:' + infoAPIPort + '/api/v1.0/info'
      const reqOptions = urlParse.parse(url)
      http
    ...
    if (retries > 0) {
      ...
      try {
        ...
        if (response.data && response.data.data && response.data.data.id) {
          const tunnelID = response.data.data.id
...
async function runBinaryV2_ (self, retries) {
  ...
  try {
    if (retries >= 0) {
      ...
      const subprocess = childProcess.spawn(self.binaryPath, binaryArguments, {
        detached: true,
        stdio: 'ignore'
      })
      subprocess.unref()

      const indexOfInfoAPIPort = binaryArguments.indexOf('--infoAPIPort')

      if (indexOfInfoAPIPort != -1 && indexOfInfoAPIPort < binaryArguments.length) {
        const infoAPIPortValue = binaryArguments[indexOfInfoAPIPort + 1]
        const tunnelID = await getTunnelID(infoAPIPortValue, maxCheckTunnelStatusRetries)
        if (tunnelID) {
