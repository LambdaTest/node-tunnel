    ...
    if (typeof fnCallback !== 'function') {
      return new Promise(function(resolve, reject) {
        ...
        if (!options['user'] || !options['key']) {
          reject({ message: 'user and key is required' });
        }
        // Configure looger
        Config_(function(error, response) {
          ...
          if (response.jsonResponse.supportedVersions.indexOf(packageVersion) === -1) {
            throw new Error(
              "\nIt's seems you have unsupported version of @lambdatest/node-tunnel." +
                'Please remove and install newer version \n ' +
                'npm uninstall @lambdatest/node-tunnel \nnpm i @lambdatest/node-tunnel'
            );
          } else if (packageVersion !== response.jsonResponse.latest) {
            console.warn(
              "\nIt's seems you have older version of @lambdatest/node-tunnel." +
  ...
  this.getTunnelName = function(fnCallback) {
    if (typeof fnCallback !== 'function') {
      ...
      return new Promise(function(resolve, reject) {
        if (that.isRunning()) {
          if (that.infoAPIPort) {
            retryTunnelName_(that, that.infoAPIPort, 5, function(tunnelName) {
              if (tunnelName === null) {
                reject(null);
              } else {
                resolve(tunnelName);
    ...
    if (this.isRunning()) {
      if (this.infoAPIPort) {
        retryTunnelName_(this, this.infoAPIPort, 5, fnCallback);
      } else {
    ...
    request.post(
      {
        ...
        url: httpTunnelConfig.jsonResponse.AuthUrl,
        body: JSON.stringify({
