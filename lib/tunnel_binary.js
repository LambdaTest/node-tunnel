var https = require("https"),
  ...
  httpHashPath = Config.tunnelBinary.linux.httpHashPath,
  localHashFilename = Config.tunnelBinary.linux.localHashFilename;
/**
...
function TunnelBinary() {
  ...
  if (this.hostOS.match(/darwin|mac os/i)) {
    this.httpPath = Config.tunnelBinary.mac.httpPath;
    this.binaryName = Config.tunnelBinary.mac.binaryName;
    httpHashPath = Config.tunnelBinary.mac.httpHashPath;
    localHashFilename = Config.tunnelBinary.mac.localHashFilename;
  } else if (
    this.hostOS.match(/mswin|msys|mingw|cygwin|bccwin|wince|emc|win32/i)
  ) {
    this.windows = true;
    this.httpPath = Config.tunnelBinary.win.httpPath;
    this.binaryName = Config.tunnelBinary.win.binaryName;
    httpHashPath = Config.tunnelBinary.win.httpHashPath;
    localHashFilename = Config.tunnelBinary.win.localHashFilename;
  } else {
    if (this.is64bits) {
      this.httpPath = Config.tunnelBinary.linux.httpPath;
      this.binaryName = Config.tunnelBinary.linux.binaryName;
    } else {
      this.httpPath = Config.tunnelBinary.linux.httpPath;
      this.binaryName = Config.tunnelBinary.linux.binaryName;
    }
  ...
  this.orderedPaths = [
    path.join(this.homeDir_(), ".lambdatest"),
    process.cwd(),
    os.tmpdir()
  ];
}
