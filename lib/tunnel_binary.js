var localTunnelConfig_ = require("./conf/localTunnelConfig.json"),
  logger,
...
function TunnelBinary(httpTunnelConfig) {
  ...
  this.httpPath = this.httpTunnelConfig.jsonResponse.tunnelBinary[
    this.platform
  ][this.bits].httpPath;
  this.binaryName = this.httpTunnelConfig.jsonResponse.tunnelBinary[
    this.platform
  ][this.bits].binaryName;
  this.httpHashContents = this.httpTunnelConfig.jsonResponse.tunnelBinary[
    this.platform
  ][this.bits].hash;
  this.localHashContents =
    localTunnelConfig_.tunnelBinary[this.platform][this.bits].hash;
    ...
    try {
      ...
      if (this.checkPath_(binaryPath, fs.X_OK)) {
        ...
        if (this.httpHashContents === this.localHashContents) {
          ...
          fs.writeFileSync(
            __dirname + "/conf/localTunnelConfig.json",
