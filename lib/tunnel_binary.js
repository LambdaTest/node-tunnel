let localTunnelConfig_ = require('./cfg/node-tunnel-config-v3-latest.json')
let logger
...
function TunnelBinary (httpTunnelConfig, options) {
  ...
  this.download_ = function (conf, destParentDir, retries, fnCallback) {
    try {
      ...
      const options = urlParse.parse(this.httpPath)
      const proxyOpts = util.getProxyOpts_(conf)
        ...
        .get(options, function (response) {
          ...
          fileStream.on('close', function () {
            if (self.checkPath_(binaryPath)) {
              const unzipBinaryPath = path.join(destParentDir, executableName)
              let destBinaryName = executableName
              if (self.windows) {
                destBinaryName += '.exe'
              }
              const destBinaryPath = path.join(destParentDir, destBinaryName)
  ...
  this.binaryPath_ = function (conf, fnCallback) {
    try {
      const destParentDir = this.availableDirs_()
      let destBinaryName = executableName
      if (this.windows) {
        destBinaryName += '.exe'
      }
      const binaryPath = path.join(destParentDir, destBinaryName)
  ...
  this.availableDirs_ = function () {
    const orderedPathLength = this.orderedPaths.length
    let _path
    let iCounter = 0
    for (let i = 0; i < orderedPathLength; i++) {
      iCounter++
